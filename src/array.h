...
static inline int array_copy(struct array_s *d, struct array_s *s)
{
	if (0 == array_init(d, s->sz, s->used)) {
		return -1;
	}
...
static inline int array_append(struct array_s *d, struct array_s *s)
{
	if (d->sz != s->sz) {
		return -1;
	}
